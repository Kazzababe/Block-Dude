...
public class Player extends Entity {
	...
	public Player(Level parentLevel, int id) {
		...
		try {
			this.sheet = new SpriteSheet("textures/BlockDudeSheet.png", 32, 64);
			this.animation = new Animation(this.sheet, BlockDude.TARGET_FPS);
	...
	public void render(float delta){
		...
		this.animation.draw(this.pos.x, this.pos.y, 11.45F, 28);
		
		//TextureHelper.drawTexture(tex, Mouse.getX(), Display.getHeight() - Mouse.getY(), this.shape.getWidth(), this.shape.getHeight());
	}
	...
	public void update(float delta) {
		...
		if (Math.abs(this.motion.x) <= 0.25) {
			this.motion.x = 0;
		}
		
		if (BlockDude.getCurrentFrame() % (BlockDude.TARGET_FPS / 4) == 0 && this.motion.x != 0) {
